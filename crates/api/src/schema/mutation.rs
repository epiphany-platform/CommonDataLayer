...
use crate::schema::utils::get_view;
...
impl Mutation {
    async fn add_schema(context: &Context, new: NewSchema) -> FieldResult<FullSchema> {
    ...
    async fn update_schema(
        ...
        async move {
            ...
            let schema_type: Option<rpc::schema_registry::types::SchemaType> =
                update.schema_type.map(|st| st.into());
            conn.update_schema(rpc::schema_registry::SchemaMetadataUpdate {
                ...
                patch: rpc::schema_registry::SchemaMetadataPatch {
                    ...
                    schema_type: schema_type.map(|st| st as i32),
                },
            })
            .await?;
    ...
    async fn update_view(context: &Context, id: Uuid, update: ViewUpdate) -> FieldResult<View> {
        let span = tracing::trace_span!("update_view", ?id, ?update);
        async move {
            let mut conn = context.connect_to_registry().await?;

            let (update_fields, fields) = if let Some(fields) = update.fields {
                (true, serde_json::from_str(&fields)?)
            } else {
