...
class EdgeRegistry:
    def __init__(self, consumer_config, postgres_config: PostgresConfig, communication_port=50110):
    ...
    def start(self):
        ...
        if type(self.consumer_config) is KafkaInputConfig:
            env.update(
                METHOD='kafka',
                MQ_HOST=self.consumer_config.brokers,
                MQ_TAG=self.consumer_config.group_id,
