...
def test_get_relation(prepare):
    ...
    relation_id = prepare.AddRelation(SchemaRelation(parent_schema_id=parent_schema_id, child_schema_id=child_schema_id)).relation_id

    result = prepare.GetRelation(RelationQuery(relation_id=relation_id, parent_schema_id=parent_schema_id)).child_schema_id

    assert child_schema_id == result

def test_get_relations(prepare):
    parent_schema_id = "1d1cc7a5-9277-48bc-97d3-3d99cfb63300"
    relation1 = prepare.AddRelation(SchemaRelation(parent_schema_id=parent_schema_id, child_schema_id="1d1cc7a5-9277-48bc-97d3-3d99cfb63301")).relation_id
    relation2 = prepare.AddRelation(SchemaRelation(parent_schema_id=parent_schema_id, child_schema_id="1d1cc7a5-9277-48bc-97d3-3d99cfb63302")).relation_id

    result = list(map(lambda x: x.relation_id, prepare.GetSchemaRelations(SchemaId(schema_id=parent_schema_id)).items))
...
def test_add_get_edge(prepare):
    ...
    relation = prepare.AddRelation(SchemaRelation(parent_schema_id=parent_schema_id, child_schema_id=child_schema_id)).relation_id
    ...
    prepare.AddEdges(ObjectRelations(relations=[Edge(relation_id=relation, parent_object_id=parent_object_id, child_object_ids=[child1, child2])]))

    result = prepare.GetEdge(RelationIdQuery(relation_id=relation, parent_object_id=parent_object_id)).child_object_ids

    assert result == [child1, child2]

def test_get_edges(prepare):
    relation1 = prepare.AddRelation(SchemaRelation(parent_schema_id="1d1cc7a5-9277-48bc-97d3-3d99cfb63000", child_schema_id="1d1cc7a5-9277-48bc-97d3-3d99cfb63001")).relation_id
    relation2 = prepare.AddRelation(SchemaRelation(parent_schema_id="1d1cc7a5-9277-48bc-97d3-3d99cfb6300c", child_schema_id="1d1cc7a5-9277-48bc-97d3-3d99cfb6300a")).relation_id
    ...
    prepare.AddEdges(ObjectRelations(relations=[
        Edge(relation_id=relation1, parent_object_id=parent, child_object_ids=[child1]),
        Edge(relation_id=relation2, parent_object_id=parent, child_object_ids=[child2])
    ]))

    result = list(map(lambda x: (x.relation_id, x.child_object_ids), prepare.GetEdges(ObjectIdQuery(object_id=parent)).relations))
